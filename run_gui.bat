...
pythonw -c "import sys; sys.exit(0 if sys.version_info >= (3, 8) else 1)" >nul 2>nul && (start "" pythonw "%~dp0codex_routing_webui.py" & exit /b 0)
python -c "import sys; sys.exit(0 if sys.version_info >= (3, 8) else 1)" >nul 2>nul && (start "" python "%~dp0codex_routing_webui.py" & exit /b 0)
