...
2. **Check**를 누릅니다. 모델은 `~/.codex/config.toml`에서, 로그인은 Codex 것을 그대로 씁니다.
3. 30초쯤 뒤 배너를 읽습니다.
...
- **모델 / 대조군**: *모델*은 검사하고 싶은 모델, *대조군*은 비교용으로 바로 이어서 검사하는
  두 번째 모델입니다(기본 `gpt-5.6-sol`). 대조군은 멀쩡한데 내 모델만 다르게 응답하면 "그
  모델만 바꿔치기"라는 뜻이고, 계정이나 클라이언트 문제와 구분됩니다. 빠르고 싸게 확인만
  하려면 `(없음)`으로 두세요.
