@echo off
rem Build dist\codex-model-check.exe (single file, no console window) with PyInstaller.
...
python -m PyInstaller --noconfirm --clean --onefile --windowed --name codex-model-check codex_model_check_gui.py || (echo PyInstaller failed. & pause & exit /b 1)
echo.
echo Built: %~dp0dist\codex-model-check.exe
pause
